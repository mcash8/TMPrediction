...
''' 
    Build the GEANT Topology
'''
def GeantTopo(): 
    '''
        Build networkx graph from geant topology xml file
    '''
    ...
    tree = ET.parse('GEANT\\data\\topology-anonymised.xml')
    root = tree.getroot()
    link_ids = []
    ...
    for link_element in igp_links:
        ...
        G.add_edge(int(node1), int(node2), capacity = 1000000000.00, cost = 1) #assuming the cost is 1 for now
        G.add_edge(int(node2), int(node1), capacity = 1000000000.00, cost = 1) #assuming the cost is 1 for now
...
def mlu_on_preds(model_outputs, num_nodes, capacity=10, topo = 'abilene'):
    ...
    if topo == 'abilene': 
        ...
        for i in range(len(model_outputs)):
            ...
            D = model_outputs[i, :].reshape(num_nodes,num_nodes)
            np.fill_diagonal(D, 0)
        ...
        for i in range(len(model_outputs)):
            ...
            D = model_outputs[i, :].reshape(23,23)
            np.fill_diagonal(D, 0)
        ...
        ValueError('Need a valid topology')
